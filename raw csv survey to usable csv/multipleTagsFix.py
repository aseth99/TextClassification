...
for x in numpy_array:
	...
	if str((x[2])) != "nan":
		...
		newTexts.append(x[1])
		newTopicsArr.append(x[2])
	if str((x[3])) != "nan":	
		newArticleIdsArr.append(x[0])
		newTexts.append(x[1])
		newTopicsArr.append(x[3])
	if str((x[4])) != "nan":	
		newArticleIdsArr.append(x[0])
		newTexts.append(x[1])
		newTopicsArr.append(x[4])
	# print(str(x[5]))
	# print(type(x[5]))
	if str((x[5])) != "nan":	
		newArticleIdsArr.append(x[0])
		newTexts.append(x[1])
		newTopicsArr.append(x[5])


a = np.array(newArticleIdsArr)
b = np.array(newTopicsArr)
c = np.array(newTexts)
print(len(a))
print(len(b))
print(len(c))
...
for x, y, z in zip(a,b,c):
	print("article ID: " + str(x) + "    topic: " + str(y) + "    text: " + str(z[:20]) + "   ....")


df = pd.DataFrame({"Article IDs" : a, "Topic" : b, "Text": c})
df.to_csv("allTopics.csv", index=False)
